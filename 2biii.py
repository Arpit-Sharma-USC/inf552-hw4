...
zero_one=zero_one_loss(Y_test_family,preds)
...
print('Predicting...')
preds_ge = clf.predict(X_test)
# print(preds)
test_score_genus = accuracy_score(Y_test_genus, preds_ge.reshape(len(preds_ge),1))
print("Test Score:", test_score_genus)
test_error_genus = 1 - test_score_genus
hamming_genus=hamming_loss(Y_test_genus, preds_ge)
print('Hamming Loss:',hamming_genus)
zero_one=zero_one_loss(Y_test_genus,preds_ge)
# print('Zero-one loss:',zero_one)
...
zero_one=zero_one_loss(Y_test_species,preds_sp)
# print('Zero-one loss:',zero_one)
...
for i in range(0,len(Y_test_species)):
    if((preds[i]==Y_test_family[i]) and (preds_sp[i]==Y_test_species[i]) and (preds_ge[i]==Y_test_genus[i])):
